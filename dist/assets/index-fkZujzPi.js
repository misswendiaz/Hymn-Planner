(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=null;async function t(){if(e)return e;try{let t=await fetch(`/Hymn-Planner/json/moods.json`);if(!t.ok)throw Error(`Failed to load moods.json (HTTP ${t.status})`);let n=await t.json();if(!n||!Array.isArray(n.moods))throw Error(`Invalid moods.json format: expected { moods: [] }`);return e=Object.freeze([...n.moods]),e}catch(e){throw console.error(`Error loading moods: `,e),e}}async function n(e,n=`Mood`,r=``){let i=await t(),a=(r||``).toLowerCase();return`
  ...
  </section>`;let t=e.querySelector(`#mood-container`);t.innerHTML=await n(`mood`,`Select Mood`);let r=e.querySelector(`#generateButton`);r.addEventListener(`click`,async()=>{if(!S){S=!0,r.textContent=`Generating...`;try{let t={topic1:e.querySelector(`#topic1`)?.value?.trim()||``,topic2:e.querySelector(`#topic2`)?.value?.trim()||``,topic3:e.querySelector(`#topic3`)?.value?.trim()||``,mood:e.querySelector(`#mood`)?.value?.trim()||``};x=t;let n=await d(t);console.log(`RAW PLAN:`,n),b={opening:n?.opening??null,sacrament:n?.sacrament??null,intermediate:n?.intermediate??null,closing:n?.closing??null};let r=e=>e?.title??`No match`;e.querySelector(`#opening`).textContent=r(b.opening),e.querySelector(`#sacrament`).textContent=r(b.sacrament),e.querySelector(`#intermediate`).textContent=r(b.intermediate),e.querySelector(`#closing`).textContent=r(b.closing)}catch(e){console.error(`Generation failed:`,e),alert(`Failed to generate hymn plan. Check console.`)}finally{S=!1,r.textContent=`Generate Hymns`}}}),e.querySelector(`#saveButton`).addEventListener(`click`,()=>{if(!b){alert(`No generated plan to save yet.`);return}let e={id:`plan-${Date.now()}`,createdAt:new Date().toISOString(),input:x||{},hymns:{opening:b.opening?.id??null,sacrament:b.sacrament?.id??null,intermediate:b.intermediate?.id??null,closing:b.closing?.id??null}};console.log(`Saving Plan:`,e),_(e)}),e.querySelector(`#printButton`).addEventListener(`click`,()=>{window.print()})}var w=`AIzaSyDzaaAKxe0qt1xDLQVxIs7CPrsn0IHfbRk`;async function T(e){try{let t=new URL(`https://www.googleapis.com/youtube/v3/search`);t.searchParams.set(`part`,`snippet`),t.searchParams.set(`q`,e),t.searchParams.set(`type`,`video`),t.searchParams.set(`maxResults`,`1`),t.searchParams.set(`videoEmbeddable`,`true`),t.searchParams.set(`key`,w),console.log(`REQUEST URL:`,t.toString());let n=await fetch(t),r=await n.json();return n.ok?r.items?.length?{videoId:r.items[0].id.videoId}:null:(console.error(`YouTube API Error:`,r),null)}catch(e){return console.error(`YouTube fetch failed:`,e),null}}var E=null,D=[],O=[],k=``,A=`all`,j=``,M=new Map;async function N(e){E=e,D=await s(),O=[...D],e.innerHTML=`
  ...
  `,ne(e),P(e.querySelector(`#hymnGrid`)),window.addEventListener(`click`,e=>{let t=e.target.closest(`[data-action='view']`),n=e.target.closest(`[data-action='use']`),r=e.target.closest(`[data-action='refresh']`);t&&oe(t.dataset.id),n&&ae(n.dataset.id),r&&z(r.dataset.id)}),R(e),F(e)}function P(e){e.addEventListener(`mouseenter`,e=>{let t=e.target.closest(`.hymn-header`);if(!t)return;let n=t.closest(`.hymn-card`);n&&n.classList.add(`title-hover`)},!0),e.addEventListener(`mouseleave`,e=>{let t=e.target.closest(`.hymn-header`);if(!t)return;let n=t.closest(`.hymn-card`);n&&n.classList.remove(`title-hover`)},!0)}function ne(e){let t=e.querySelector(`#searchInput`),n=e.querySelector(`#filterType`),r=e.querySelector(`#filterValue`),i=e.querySelector(`#applyFilter`),a=e.querySelector(`#clearFilter`);t.addEventListener(`input`,t=>{k=t.target.value.toLowerCase(),F(e)}),n.addEventListener(`change`,e=>{A=e.target.value}),r.addEventListener(`input`,e=>{j=e.target.value.toLowerCase()}),i.addEventListener(`click`,()=>F(e)),a.addEventListener(`click`,()=>{k=``,A=`all`,j=``,t.value=``,r.value=``,n.value=`all`,F(e)})}async function F(e){let t=e.querySelector(`#hymnGrid`);O=D.filter(e=>(e.title.toLowerCase().includes(k)||String(e.number).includes(k))&&re(e)),t.innerHTML=``;let n=0;function r(){let e=O.slice(n,n+50).map(ie).join(``);t.insertAdjacentHTML(`beforeend`,e),P(t),n+=50,n<O.length&&requestAnimationFrame(r)}r(),requestAnimationFrame(()=>{document.querySelectorAll(`.video-container`).forEach(e=>{let t=e.id.replace(`video-`,``),n=D.find(e=>e.id===t);n&&B(n)})})}function re(e){if(A===`all`)return!0;let t=j.trim();return t?A===`topic`?(e.topics||``).toLowerCase().includes(t):A===`keyword`?(e.keywords||``).toLowerCase().includes(t):A===`mood`?(e.mood||``).toLowerCase().includes(t):!0:!0}function ie(e){return`
  ...
  `}function ae(e){let t=D.find(t=>t.id===e);t&&I(t)}function oe(e){let t=D.find(t=>t.id===e);if(!t)return;let n=E.querySelector(`#viewModal`),r=n.querySelector(`#viewTitle`),i=n.querySelector(`#viewMeta`),a=n.querySelector(`#openExternalLinks`),o=n.querySelector(`#closeView`);r.textContent=`${t.number}. ${t.title}`,i.innerHTML=`
  ...
  `;let s=t.lyrics_link;a.onclick=()=>{s&&window.open(s,`_blank`)},a.textContent=`View Lyrics & Sheet Music`,o.onclick=()=>{n.classList.add(`hidden`)},n.classList.remove(`hidden`)}function I(e){let t=E.querySelector(`#useModal`),n=t.querySelector(`#modalHymnTitle`),r=t.querySelector(`#modalPlan`),i=g();n.textContent=`${e.number}. ${e.title}`,r.innerHTML=`
  ...
  `,t.classList.remove(`hidden`),L(t,e)}function L(e,t){let n=e.querySelector(`#confirmUse`),r=e.querySelector(`#cancelUse`),i=e.querySelector(`#modalSlot`),a=e.querySelector(`#modalPlan`),o=()=>e.classList.add(`hidden`);n.onclick=()=>{let e=i.value,n=a.value,r;if(n===`__new__`)r={id:`plan-${Date.now()}`,createdAt:new Date().toISOString(),input:{},hymns:{opening:null,sacrament:null,intermediate:null,closing:null}};else if(r=g().find(e=>e.id===n),!r){alert(`Selected plan no longer exists.`);return}r.hymns=r.hymns||{},r.hymns[e]=t.id,y(r),window.dispatchEvent(new CustomEvent(`plans:updated`,{detail:{planId:r.id}})),o()},r.onclick=o}function R(e){e.querySelector(`#useModal`)||e.insertAdjacentHTML(`beforeend`,`
    ...
    <iframe
      ...
      loading="lazy"
    ></iframe>
  `}var V=null,H=[],U=new Set,W={opening:`Opening`,sacrament:`Sacrament`,intermediate:`Intermediate`,closing:`Closing`},G=e=>W[e]||e;function K(){q(),J(V)}function q(){let e=g();H=JSON.parse(JSON.stringify(e||[])),H=H.map(e=>({...e,hymns:Object.fromEntries(Object.entries(e.hymns||{}).map(([e,t])=>[e,t&&typeof t==`object`?t.id:t??null]))}))}function se(e){V=e,e.innerHTML=`
    <section class="plans">
        <h1>Saved Hymn Plans</h1>
        
        <!-- Container where plans will be rendered -->
        <div id="plansContainer"></div>
    </section>`,q(),J(V)}async function J(e){let t=e.querySelector(`#plansContainer`),n=await s();if(H.length===0){t.innerHTML=`<p>No saved plans yet.</p>`;return}t.innerHTML=H.map(e=>ce(e,n)).join(``),Z(e,n)}function ce(e,t){let n=U.has(e.id);return`
    <div class="plan-card" data-plan="${e.id}">
          ...
          ${Y(e,`closing`,t,n)}
      </div>`}function le(e,t){let n=u[t];return n?e.filter(e=>{let r=String(e.topics||``).split(`,`).map(e=>e.trim().toLowerCase()).filter(Boolean),i=String(e.keywords||``).split(`,`).map(e=>e.trim().toLowerCase()).filter(Boolean);if(t===`sacrament`)return(n.strictTopics||[]).map(e=>e.toLowerCase()).some(e=>r.some(t=>t.includes(e))||i.some(t=>t.includes(e)));let a=[...r,...i];return(n.topics||[]).some(e=>{let t=e.toLowerCase();return a.some(e=>e.includes(t))})}):e}function Y(e,t,n,r){let i=le(n,t),a=String(e.hymns?.[t]??``);return`
  <div class="slot">
    <label>${G(t)}</label>
...
`}var X=null;function Z(e){let t=e.querySelector(`#plansContainer`);X!==t&&(X=t,t.addEventListener(`click`,e=>{let t=e.target.closest(`button`);if(!t)return;let n=t.dataset.id;switch(t.dataset.action){case`edit`:ue(n),K();break;case`save`:de(n),K();break;case`reset`:fe(n),K();break;case`print`:pe(n);break;case`delete`:te(n),K();break}}),t.addEventListener(`change`,e=>{let t=e.target.closest(`select`);if(!t)return;let n=t.dataset.plan,r=t.dataset.slot,i=H.find(e=>e.id===n);i&&(i.hymns||={},i.hymns[r]=String(t.value))}))}function ue(e){U.has(e)?U.delete(e):U.add(e)}function de(e){let t=H.find(t=>t.id===e);t&&(y(t),U.delete(e),K())}function fe(e){let t=g().find(t=>t.id===e),n=H.find(t=>t.id===e);!n||!t||(n.hymns=Object.fromEntries(Object.entries(t.hymns||{}).map(([e,t])=>[e,t&&typeof t==`object`?t.id:t??null])),U.delete(e),K())}function pe(e){let t=document.querySelector(`[data-plan="${e}"]`);t&&(document.body.classList.add(`print-single-plan`),document.querySelectorAll(`.plan-card`).forEach(e=>e.classList.remove(`print-target`)),t.classList.add(`print-target`),setTimeout(()=>{window.print()},50))}var Q=document.querySelector(`#content`);if(!Q)throw Error(`Root container #content not found!`);function me(e=`generator`){Q.innerHTML=``,e===`generator`?C(Q):e===`library`?N(Q):e===`plans`?se(Q):C(Q)}function he(){let e=document.querySelector(`nav`);e&&e.addEventListener(`click`,e=>{let t=e.target.closest(`a`);if(!t)return;e.preventDefault();let n=t.dataset.page;n&&(window.location.hash=n)})}he();function ge(){return window.location.hash.replace(`#`,``)||`generator`}function $(){me(ge())}window.addEventListener(`hashchange`,$),window.addEventListener(`load`,$),$();